...
// 这是一个属性是用来放一些函数的
const mutations = {
  storePic(state,status){
    state.imglist=status;
  },
  add (state, num=9) {
    state.count += num ;
  },
...
//mutation方法才可以改变state里面的参数
//vuex相当于一个池子通过下标在池子里面找到
//相当于computed计算属性
const getters = {
  count1 (state) {
...
// 这个action也是一个函数但是里面的函数最终是提交给上面的mutation来处理，action是异步的
const actions = {
  //action里面的动作提交给mutation
  storePic({commit},status){
    commit('storePic',status);
  },
